...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      ...
      return NextResponse.json({
        ...
        analysis: {
          ...
          totalFat: 15,
          confidence: 'medium',
          description: 'Demo-modus: Legg til OPENAI_API_KEY i Vercel Environment Variables for ekte AI-analyse.',
        }
      });
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to analyze image', details: error }, { status: 500 });
    }
